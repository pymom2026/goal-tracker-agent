...
function detectConflicts(events, dayName) {
  ...
  for (const event of events) {
    if ((event.description || '').includes('Added from Goal Tracker')) continue;
    if (/^[📚🏃💪🧘]/.test(event.summary || '')) continue;
    const evMins = getEventMins(event);
    if (!evMins) continue; // skip all-day events

    const eventType = classifyEvent(evMins.title);
    for (const w of exerciseWindows) {
      if (eventType === 'exercise') continue; // event IS exercise — not a conflict
    ...
    for (const w of studyWindows) {
      if (eventType === 'study') continue; // event IS study — not a conflict
      if (overlaps(evMins, w.start, w.end)) {
